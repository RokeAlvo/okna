<template>
  <div class="wrapper-search-on-map">
    <Menu></Menu>
	<div class="map-search-wrapper" id="map-search-wrapper" @scroll="preloadLayouts">
    
    <div class="mobile-header">
      <div class="mob-main-head">
        <img @click="openMenu" class="mob-menu-open" src="/images/map-search/burger.svg">
        <img src="/images/map-search/logo-mob.svg">
        
        <div class="mob-name-search-comp">
          <InputSearch @show-by-name="showFiltredByName" 
      ...
      <div class="mob-choose-title" :class="{'mob-title--hidden' : isMobTitle}">
        <div @click="chooseView(1)" class="mob-choose" :class="{'mob-choose--active': viewActive ===1}">Список</div>
        <div @click="chooseView(2)" class="mob-choose" :class="{'mob-choose--active': viewActive ===2}">Карта</div>
      </div>
    </div>

    <div class="tablet-head-fake"></div>

    <div @click="openMobFilters" class="open-filter-btn">
      <img src="/images/map-search/filters-white.svg">
      <div v-if="!countFilters || !residentials.length" class="mob-filter-title">Фильтры</div>
      <div v-else class="mob-filters-count">{{countFilters}}</div>
    </div>

    <div class="mob-filters-wrapper" :class="{'mob-filter-open' : isOpenMobFilers}">
      <filterRC @show-rc="showFiltredRc" 
        @mob-title="mobTitleOpen"
        @count-filters="handlerCountFilters"
      ></filterRC></div>
		<div class="search-wrapper" :class="{'top-scroll' : isMobTitle, 'changed-view-list': viewActive === 2 }">
			<!-- <div> -->
			<div class="pc-filters-wrapper"><filterRC @show-rc="showFiltredRc" 
        @drop-name="dropChildSearchName"></filterRC></div>
      <div id="all-layouts-wrapper" class="all-layouts-wrapper" @scroll="preloadLayouts">
        <div v-if="residentials.length" class="mob-filtred-count">
				    <span>Показано {{filtredRC.length}}</span>
				    из {{residentials.length}} объектов
			  </div>
			  <Layouts @residential-data="toggleMarkerPopup($event)" :lazy-residentials="lazyResidentials"></Layouts>
      </div>
		</div>
		<div class="map-wrapper" :class="{'top-scroll' : isMobTitle, 'changed-view-map': viewActive === 2, 'changed-z-map': isSetZIndex}">
			<div class="map-box">
				<div class="zoom-control-wrapper">
					<div @click="zoom(+1)" class="zoom-control zoom-plus">
						<img src="/images/map-search/plus-gray.svg">
					</div>
					<div @click="zoom(-1)" class="zoom-control zoom-minus">
						<img src="/images/map-search/minus-gray.svg">
					</div>
				</div>
        <div class="name-search-comp" :class="{'name-search-comp--open-card': isOpenCard}">
          <InputSearch ref="search" @show-by-name="showFiltredByName" 
            @apply-filter="showFiltredRc"
             @open-mob-search="openMobSearch"
             @close-name-search="closeMobSearch"
          >
      ...
      ref="popup"
			...
			:developer="getCurrentResidential? getCurrentResidential.developer.name : ''"
			...
			:date-min="getCurrentResidential? getCurrentResidential.completion_deadline_min : {}"
			:date-max="getCurrentResidential? getCurrentResidential.completion_deadline_max : {}"
			:ranges="getCurrentResidential? getCurrentResidential.apartments_formatted: []"
			:coordinates="getCurrentResidential? [getCurrentResidential.latitude,getCurrentResidential.longitude]: []"
...
import filterRC from "./modules/filter";
...
export default {
    ...
    openMobSearch(){
      ...
      this.isMobNameOpen = true
		...
		preloadLayouts() {
  if(!this.isMobNameOpen)  //если открыт поиск по имени
{
  var scrolled = document.querySelector('#map-search-wrapper').scrollTop;
  if(window.innerWidth<500) scrolled = document.querySelector('#all-layouts-wrapper').scrollTop;
  ...
  this.lastScrollTop = scrolled;
      ...
      if(document.querySelector('.map-search-wrapper').scrollTop > document.documentElement.clientHeight){
        this.setAccordeonClassFilters(false)
        this.setIsOpenAllFilters(false)
      }

      if(!this.residentials.length){ return; }
      let heightOfWindow = document.getElementById("map-search-wrapper").scrollTop + window.innerHeight;
      let bottomOfWindow = heightOfWindow + 1000 >= document.getElementById("map-search-wrapper").scrollHeight;
      if (bottomOfWindow) {
      ...
      setTimeout(()=>this.$refs.popup.setMarker(),250);
		...
		setMiniMarker(data) {
			let priceMinFormatted = Number(data.price_min).toLocaleString("ru");
      ...
      rcs.forEach(residential => {
        ...
        this.map.fitBounds(this.markers.getBounds());
          ...
          this.setIsOpenCard(true)
					...
					if(window.innerWidth<500) //для избежания ошибки отсутсвия маркера
      			setTimeout(()=>this.$refs.popup.setMarker(),250);
    ...
    openMenu(){
      ...
      document.querySelector('#nav-toggle').checked = !document.querySelector('#nav-toggle').checked
    },
    chooseView(view){
    ...
    fetchData() {
      this.dataRequest.then(response => {
		...
		...mapActions(["getAllData","filter"]),
		...mapMutations(["getSelectedResidential", "setResidentialId",
        'setIsOpenAllFilters',
  ...
  updated(){
		// console.log('getCurrentResidential', this.getCurrentResidential);
    ...
    // console.log(this.maxPriceFilter);
    let _desc =
      "Удобный поиск новостроек без комиссии";
