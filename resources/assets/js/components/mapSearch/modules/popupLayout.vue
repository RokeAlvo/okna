<template>
	<div class="popup-rc-wrapper" :class="{'popup-rc-active': isOpenRc, 'popup-rc-two-active' : isReOpenRc}">

    <div class="mob-popup-header">
      <div @click="close" class="mob-popup-back"><img src="/images/map-search/back.svg"></div>
      <div class="mob-popup-title">
		...
		<div @click="close" class="popup-rc-layout-close">
		...
		<div class="popup-rc-content">
			...
			<div class="popup-rc-main-info">
				...
				<div class="popup-rc-item">
					...
					<div class="popup-rc-main-data">
						...
						<div class="popup-rc-data-value">{{dateMin.key === dateMax.key? dateMin.value : dateMin.value +' - '+ dateMax.value}}</div>
					</div>
				</div>
			</div>
			...
			<div class="popup-rc-more">
				...
				<div class="popup-rc-more-info">
					<div v-for="(range, index) in ranges" :key="index" class="popup-rc-more-item">
						  <div class="popup-more-rooms">{{range.title}}</div>
						  <div class="popup-more-square">
						  	{{Math.round(range.area_min)}} – {{Math.round(range.area_max)}} м
						  	<sup>2</sup>
						  </div>
						  <div class="popup-more-price">от {{Number(range.price_min).toLocaleString("ru")}} ₽</div>
					</div>
				</div>
			</div>
      ...
      <div class="mob-map-wrapper" :class="{'mob-popup-with-map': viewActive === 2}">
				  <div class="mob-zoom-control-wrapper">
				  	<div @click="zoom(+1)" class="mob-zoom-control">
				  ...
				  <div id="map-popup" class="mob-maps" style="width:100%; height:100%"></div>
      </div>



			<div class="popup-btn-rc-wrapper" :class="{'popup-btn-rc-wrapper--map' : viewActive === 2}">
        <div class="btb-rc-box">
				  <ButtonRC @click="linkUp" :text="'Подробно о комплексе'" :size="'m'" :width="'100%'" :value="'main'"></ButtonRC>
        </div>
			</div>
		</div>
	</div>
...
const { mapState, mapGetters, mapActions, mapMutations } = createNamespacedHelpers('mapSearch')
...
export default {
  ...
  // },
    ...
    setMiniMarker() {
			let priceMinFormatted = Number(this.priceMin).toLocaleString("ru");
...
.btb-rc-box{
  background: linear-gradient(180deg, rgba(241, 241, 241, 0) 0%, rgba(241, 241, 241, 0.89) 68.51%, #F1F1F1 100%, #F1F1F1 100%);
  padding: 0px 20px 30px;
}
