<template>
	<div class="search-btn-wrapper" :class="{'search-open': isOpenSearch}">
      ...
      <img class="mob-back-img" @click="searchClose" src="/images/map-search/back.svg">
			<input
				id="inpit-search"
      ...
      <!-- for mobile -->
      <img v-if="!inputText" class="mob-img-drop mob-img--loop" src="/images/map-search/search-black.svg">
      <img v-else @click="mobSearchDrop" class="mob-img-drop" src="/images/map-search/close-black.svg">
		</div>
		<div v-show="isOpenSearch && nameSearch.length && !choosenName" class="search-result-wrapper" id="search-box">
      <div :key="index" v-for="(residential, index) in getFilterRc.slice(0,4)" 
          class="result-point" :class="{'active-name': index === currentIndex}"
      ...
      <!-- @click="handlerDevOrRc(residential.title.toLowerCase().indexOf(nameSearch) !== -1 
          ...
          <img v-if="residential.title.toLowerCase().indexOf(nameSearch) !== -1" class="img-result" src="/images/map-search/rc-search.svg">
          <img v-else class="img-result" src="/images/map-search/developer-search.svg">
				</div>
				<div class="result-title">{{residential.title.toLowerCase().indexOf(nameSearch) !== -1 ? residential.title : residential.developer.name}}</div>
			</div>
		</div>
	</div>
</template>
...
export default {
  ...
  computed:{
	...
	methods: {
    ...mapActions(["filter"]),
    ...mapMutations(['setNameSearch','setIsOpenSearch',
    'setIsOpenAllFilters','setAccordeonClassFilters','getFiltredRcByName','getSearchableData']),
		searchOpen() {
      ...
      var inp = document.getElementsByClassName('input-search-name')
    ...
    mobSearchDrop(){
      ...
      this.setNameSearch('')
      this.choosenName = ''
      if(this.residentials !== this.filtredRC)
        this.$emit('apply-filter', 10)
        this.$emit('close-name-search')
      
		},
		inputHandler(event) {
      let nameSearch = this.inputText.toLowerCase()
    ...
    next(list) {
      let elemsArr = Array.from(list.children)
      if(!elemsArr.length) return;
      let current = elemsArr.findIndex(el => el.classList.contains('active-name'))
      let next = current + 1
  ...
  mounted(){
    let list = document.getElementsByClassName('search-result-wrapper')
  var elListen = document.getElementsByClassName('search-btn-wrapper')
  for(let i = 0; i< elListen.length; i++){
        elListen[i].addEventListener('keydown', evt => {
  switch (evt.keyCode) {
    case 38: this.prev(list[1]); break;
    case 40: this.next(list[1]); break;
    case 13 :
      if(this.choosenName
