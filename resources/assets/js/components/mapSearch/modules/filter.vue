<template>
    ...
    <div class="filter-global-wrapper fake-filter">
      <div @click="toggleAllFilters" class="filter-title-box">
					<div class="filter-main-title">
						<span v-if="!isOpenAllFilters" class="close-filter">Фильтры</span>
						<span v-if="isOpenAllFilters" class="open-filter">Свернуть фильтры</span>
					</div>
					<div class="filter-img-wrapper">
						<div v-if="countFilters" class="count-all-filters">{{countFilters}}</div>
		...
		<div class="filter-global-wrapper">
			<div class="filter-main-content">
				<div @click="toggleAllFilters" class="filter-title-box filter-title-box--natural">
					<div class="filter-main-title">
						<span v-if="!isOpenAllFiltersWith" class="close-filter">Фильтры</span>
						<span v-if="isOpenAllFiltersWith" class="open-filter">Свернуть фильтры</span>
					</div>
					<div class="filter-img-wrapper">
						<div v-if="countFilters" class="count-all-filters">{{countFilters}}</div>
						<img src="/images/map-search/filters.svg">
				...
				>
					<div class="filter-box-wrapper">
						<div class="search-result" :class="{'search-result-show': filterNumber === 1 && isShowBtn}">
							<ButtonRC :size="'xl'" :action="statusBtn" @click="showRcOnMap(getFilterRc.length)">
								<span v-if="statusBtn !== 'loading' && getFilterRc.length">Показать {{getFilterRc.length}}</span>
								<div v-else-if="statusBtn !== 'loading' && !getFilterRc.length" class="static-btn-text">
								  <div class="static-btn-text__title">Не найдено подходящих ЖК</div>
								  <div class="static-btn-text__pretext">Попробуйте изменить параметры поиска</div>
								</div>
							</ButtonRC>
						</div>

						<div class="filter-box">
							<div @click="toggleRoomsFilter" class="filter-title">
								<div class="filter-name">Количество комнат</div>
								<div class="filter-close">
									<span @click.stop="dropActiveRooms" v-if="activeRooms.length" class="pretext-drop">Сбросить</span>
									<img
							...
							<div v-show-slide="roomsFilterOpen" class="filter-content">
								<div class="filter-content-box">
									<div class="room-buttons-wrapper">
										<ButtonRC :text="'Все'" :isactive="!activeRooms.length" @click="chooseRooms(-1)"></ButtonRC>
										<ButtonRC
					...
					<div class="filter-box-wrapper">
						<div class="search-result" :class="{'search-result-show': filterNumber === 2 && isShowBtn }">
							<ButtonRC :size="'xl'" :action="statusBtn" @click="showRcOnMap(getFilterRc.length)">
								<span v-if="statusBtn !== 'loading' && getFilterRc.length">Показать {{getFilterRc.length}}</span>
								<div v-else-if="statusBtn !== 'loading' && !getFilterRc.length" class="static-btn-text">
								  <div class="static-btn-text__title">Не найдено подходящих ЖК</div>
						...
						<div class="filter-box">
							<div @click="togglePriceFilter" class="filter-title">
								<div class="filter-name">Стоимость</div>
								<div class="filter-close">
									<span v-if="!togglePrice" @click.stop="dropPrice" class="pretext-drop">Сбросить</span>
									<img
										v-if="togglePrice"
					...
					<div v-if="completionYears.length" class="filter-box-wrapper filter-box-wrapper--more">
						<div class="search-result" :class="{'search-result-show': filterNumber === 3 && isShowBtn}">
							<ButtonRC :size="'xl'" :action="statusBtn" @click="showRcOnMap(getFilterRc.length)">
								<span v-if="statusBtn !== 'loading' && getFilterRc.length">Показать {{getFilterRc.length}}</span>
								<div v-else-if="statusBtn !== 'loading' && !getFilterRc.length" class="static-btn-text">
								  <div class="static-btn-text__title">Не найдено подходящих ЖК</div>
						...
						<div class="filter-box">
							<div @click="toggleMoreFilter" class="filter-title">
								<div class="filter-name">Дополнительно</div>
								<div class="filter-close">
									<span v-if="toggleAdditionally" @click.stop="dropAdditionallyFilter" class="pretext-drop">Сбросить</span>
									<img
							...
							>
								<div class="filter-content-box">
									<div class="more-filter-wrapper">
										<div class="more-filter" :class="{'mob-active-additional': isMobDate }">
											<div class="more-filter-header" @click="mobToggleAdditional('date')">
												<div class="more-filter-title">Срок сдачи</div>
												<div class="more-filter-change">
													<img v-if="!pointsDate.length" class="open-point-filter-img" src="/images/map-search/arrow.svg">
													<div v-else class="count-filer-points">{{ pointsDate.length }}</div>
												</div>
											</div>
											<div v-show-slide="compMobDate" class="more-filter-body">
												<div class="more-filter-body-title">Выберите один или несколько</div>
                            ...
                            @click="checkedDate(point)"
													>
														<div class="checkbox-box" :class="{'checkbox-box--checked': pointsDate.includes(point) }"></div>
														<div class="more-filter-body-point-name">{{point}}</div>
													</div>
												</div>
											</div>
										</div>

										<div class="more-filter" :class="{'mob-active-additional': isMobBay }">
											<div class="more-filter-header" @click="mobToggleAdditional('bay')">
												<div class="more-filter-title">Способы покупки</div>
												<div class="more-filter-change">
													<img v-if="!pointsPaymentMethods.length"  class="open-point-filter-img" src="/images/map-search/arrow.svg">
													<div v-else class="count-filer-points">{{ pointsPaymentMethods.length }}</div>
												</div>
											</div>
                            ...
                            @click="checkedPaymentMethods(point.id)"
													>
														<div class="checkbox-box" :class="{'checkbox-box--checked': pointsPaymentMethods.includes(point.id)}"></div>
														<div class="more-filter-body-point-name">{{point.name}}</div>
													</div>
												</div>
											</div>
										</div>

										<div class="more-filter" :class="{'mob-active-additional': isMobMaterial }">
											<div class="more-filter-header" @click="mobToggleAdditional('material')">
												<div class="more-filter-title">Материал дома</div>
												<div class="more-filter-change">
													<img v-if="!pointsMaterial.length" class="open-point-filter-img" src="/images/map-search/arrow.svg">
													<div v-else class="count-filer-points">{{pointsMaterial.length}}</div>
												</div>
											</div>
											<div v-show-slide="compMobDMaterial" class="more-filter-body">
												<div class="more-filter-body-title">Выберите один или несколько</div>
												<div class="more-filter-body-point-wrapper">
													<div
														:key="index"
                            ...
                            @click="checkedMaterial(point.id)"
													>
														<div class="checkbox-box" :class="{'checkbox-box--checked': pointsMaterial.includes(point.id)}"></div>
														<div class="more-filter-body-point-name">{{point.name}}</div>
													</div>
												</div>
											</div>
										...
										<div class="more-filter" :class="{'mob-active-additional': isMobDecoration }">
											<div class="more-filter-header" @click="mobToggleAdditional('decor')">
												<div class="more-filter-title">Отделка</div>
												<div class="more-filter-change">
													<img v-if="!pointsDecoration.length"  class="open-point-filter-img" src="/images/map-search/arrow.svg">
													<div v-else class="count-filer-points">{{pointsDecoration.length}}</div>
												</div>
											</div>
											<div v-show-slide="compMobDecoration" class="more-filter-body">
												<div class="more-filter-body-title">Выберите один или несколько</div>
												<div class="more-filter-body-point-wrapper">
													<div
														:key="index"
                            ...
                            @click="checkedDecoration(point.key)"
													>
														<div class="checkbox-box" :class="{'checkbox-box--checked': pointsDecoration.includes(point.key)}"></div>
														<div class="more-filter-body-point-name">{{point.value}}</div>
													</div>
												</div>
											</div>
										</div>

										<div class="more-filter" :class="{'mob-active-additional': isMobInfrastructure }">
											<div class="more-filter-header" @click="mobToggleAdditional('infra')">
												<div class="more-filter-title">Инфраструктура</div>
												<div class="more-filter-change">
													<img v-if="!pointsInfrastructures.length" class="open-point-filter-img" src="/images/map-search/arrow.svg">
													<div v-else class="count-filer-points">{{pointsInfrastructures.length}}</div>
												</div>
											</div>
											<div v-show-slide="compMobInfrastructure" class="more-filter-body">
												<div class="more-filter-body-title">Выберите один или несколько</div>
												<div class="more-filter-body-point-wrapper">
													<div
														:key="index"
                            ...
                            @click="checkedInfrastructures(point.id)"
													>
														<div class="checkbox-box" :class="{'checkbox-box--checked': pointsInfrastructures.includes(point.id)}"></div>
														<div class="more-filter-body-point-name">{{point.name}}</div>
													</div>
												</div>
											</div>
										...
										<div class="more-filter" :class="{'mob-active-additional': isMobParking }">
											<div class="more-filter-header" @click="mobToggleAdditional('parking')">
												<div class="more-filter-title">Тип парковки</div>
												<div class="more-filter-change">
													<img v-if="!pointsParking.length" class="open-point-filter-img" src="/images/map-search/arrow.svg">
													<div v-else class="count-filer-points">{{pointsParking.length}}</div>
												</div>
											</div>
											<div v-show-slide="compMobParking" class="more-filter-body">
												<div class="more-filter-body-title">Выберите один или несколько</div>
												<div class="more-filter-body-point-wrapper">
													<div :key="index" 
                                ...
                                @click="checkedParking(point.key)">
														<div class="checkbox-box" :class="{'checkbox-box--checked': pointsParking.includes(point.key)}"></div>
														<div class="more-filter-body-point-name">{{point.value}}</div>
													</div>
												</div>
											</div>
			...
			<div v-if="!isOpenAllFilters && residentials.length" class="slide-filtred-count">
				<span>Показано {{filtredRC.length}}</span>
				из {{residentials.length}} объектов
			</div>
		</div>
    <div class="mob-count-rc">
      <ButtonRC :size="getFilterRc.length? 'm' : 'xl'" :value="getFilterRc.length ? 'main' : ''" :action="statusBtn" @click="showRcOnMap(getFilterRc.length)">
								<span v-if="statusBtn !== 'loading' && getFilterRc.length">Показать <span class="bold-text">{{getFilterRc.length}}</span> объектов</span>
								<div v-else-if="statusBtn !== 'loading' && !getFilterRc.length" class="static-btn-text">
								  <div class="static-btn-text__title">Не найдено подходящих ЖК</div>
								  <div class="static-btn-text__pretext">Попробуйте изменить параметры поиска</div>
								</div>
...
export default {
    ...
    statusBtn(){
      if(!this.getFilterRc.length) return 'disabled'
		...
		toggleAdditionally() {
			return (
        this.pointsDate.length || this.pointsPaymentMethods.length || this.pointsMaterial.length
        || this.pointsDecoration.length || this.pointsInfrastructures.length || this.pointsParking.length
			);
		},
		defaultValueLeft() {
			return (
    ...
    anyFilterActive(){
      return (this.minSum !== this.allRcMinPrice || this.maxSum !== this.allRcMaxPrice
      || this.pointsDate.length || this.pointsPaymentMethods.length || this.pointsMaterial.length
        || this.pointsDecoration.length || this.pointsInfrastructures.length || this.pointsParking.length)
    },
    countFilters(){
      ...
      if(this.minSum !== this.allRcMinPrice || this.maxSum !== this.allRcMaxPrice) count++
      ...
      this.dropPrice()
        ...
        setTimeout(()=> this.dropPrice(),200)
				...
				if (this.activeRooms.includes(index[0])) {
					let indexRoom = this.activeRooms.indexOf(index[0]);
					let lengthRooms = index.length;
    ...
    checkedDate(point){
      ...
      if(this.pointsDate.includes(point)){
        let pointIndex = this.pointsDate.indexOf(point)
    ...
    checkedPaymentMethods(id){
      if(this.pointsPaymentMethods.includes(id)){
        let idIndex = this.pointsPaymentMethods.indexOf(id)
    ...
    checkedMaterial(id){
      if(this.pointsMaterial.includes(id)){
        let idIndex = this.pointsMaterial.indexOf(id)
    ...
    checkedDecoration(id){
      if(this.pointsDecoration.includes(id)){
        let idIndex = this.pointsDecoration.indexOf(id)
    ...
    checkedInfrastructures(id){
      if(this.pointsInfrastructures.includes(id)){
        let idIndex = this.pointsInfrastructures.indexOf(id)
    ...
    checkedParking(id){
      if(this.pointsParking.includes(id)){
        let idIndex = this.pointsParking.indexOf(id)
    ...
    showRcOnMap(length){
      ...
      if(length) {
        ...
        this.setNameSearch('')
        this.$emit('mob-title') 
        if(this.screenWidth <500) document.querySelector('.all-layouts-wrapper').scrollTop=0;
  ...
  created(){
    ...
    setTimeout(()=>{
      if(!this.isIpotekaPrice) {this.dropPrice()}
      else{ 
