        ...
        new Vue({
            ...
            methods: {
                ...
                phoneChange: function () {
                    var phone = $('.header-main-phone .main-phone').first().text();
                    var tel = phone.replace(/[^0-9]/gi, '');
                    $('.header-main-phone .main-phone').attr("href", 'tel:+' + tel);
                    if ($('.popup .main-phone').length) {
                        $('.popup .main-phone:not(.text-button-popup)').html(phone);
                        $('.popup .main-phone').attr("href", 'tel:+'+ tel);
                    }
                }
            },
            components: {
    ...
    <section id="residentials" data-ga-popup-open-label="{{ $gaLabel }}">
                    ...
                    <div class="col-md-8 col-lg-9">
                        <div class="property-developer-search-filters">
                            ...
                            <div class="col-md-6">
                                ...
                                <template slot="tooltip" scope="tooltip">
                                    <div class="slider-tooltip">
                                        @{{ String(tooltip.value).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 ') }} р.
                                    </div>
                                </template>
                                </vue-slider>
                            </div>
                            <div class="property-developer-search-filters-sliders-legend">
                                <div class="property-developer-search-filters-sliders-legend-left">{{ number_format($apartmentRanges['minPrice'], 0, '.', ' ') }} р.</div>
                                <div class="property-developer-search-filters-sliders-legend-center">цена</div>
                                <div class="property-developer-search-filters-sliders-legend-right">{{ number_format($apartmentRanges['maxPrice'], 0, '.', ' ') }} р.</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-3 col-md-4">
                <div class="property-developer-search-result">
                    ...
                    <div class="property-developer-search-result-value">
                        <span class="property-developer-search-result-value-number" v-if="!loading">
                            @{{ totalApartments }}
                        </span>
                        <span class="property-developer-search-result-value-number" v-else >
                            <div class="sk-fading-circle" style="margin: 0">
        ...
        <div v-for="(residential, index) in residentials" class="property-developer-rc" v-if="residential.apartments.length">
            <div class="property-developer-rc-block">
                <div class="row">
                    <div class="col-md-8 col-lg-9">
                        <div class="property-developer-rc-left" :style="{ backgroundImage: 'url(' + residential.search_image + ')' }">
                    ...
                    <div class="col-md-4 col-lg-3">
                        <div class="property-developer-rc-right">
                            ...
                            <table>
                                ...
                                <tbody>
                                <tr v-for="range in residential.mergedRanges" v-if="range.apartments_count > 0">
                                    <td>@{{ range.room_label }}</td>
                                    <td>@{{ String(range.price_min).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 ') }}</td>
